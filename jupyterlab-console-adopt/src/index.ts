 ...
 */
const consoleAdoptPlugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (
    ...
    consoleTracker.currentChanged.connect((_, consolePanel) => {
      ...
      let currentIOPubHandler:
        | ((_: any, msg: KernelMessage.IIOPubMessage) => void)
        | null = null;
      ...
      const onKernelChanged = () => {
        ...
        const msgIdToCell = new Map<string, {
          cell: any;
        }>();

        const onIOPubMessage = (
          _: any,
          ...
          if (msgType === 'execute_input') {
            ...
            const parentHeader = msg.parent_header as any;
            const sessionId = parentHeader?.session || '';

            // 跳过 Console 自身 session 的执行
            if (sessionId === kernel.clientId) return;

            const code: string = content.code || '';
            const msgId: string = parentHeader?.msg_id || '';
            const execCount: number = content.execution_count;

            if (!code || !msgId) return;

            console.log(
              `[console-adopt] 捕获外部执行 #${execCount}: session=${sessionId.slice(0,8)}...`
            );
            ...
            // 伪 future，用于路由消息
            const capturedOutputs: KernelMessage.IIOPubMessage[] = [];

            const pseudoFuture: any = {
              onIOPub: null as ((msg: KernelMessage.IIOPubMessage) => void) | null,
              onReply: null,
              onStdin: null,
              done: new Promise<void>((resolve) => {
                const checkDone = (_: any, m: KernelMessage.IIOPubMessage) => {
                  if (
                    m.parent_header?.msg_id === msgId &&
                    m.header.msg_type === 'status' &&
                    (m.content as any).execution_state === 'idle'
                  ) {
            ...
            // 设置到 OutputArea
            (cell.outputArea as any).future = pseudoFuture;
            cell.outputHidden = false;

            consoleWidget.addCell(cell, msgId);

            // 转发后续输出
            const forwardOutput = (_: any, m: KernelMessage.IIOPubMessage) => {
              if (m.parent_header?.msg_id !== msgId) return;
              const mt = m.header.msg_type;
              if (mt === 'execute_input' || mt === 'status') {
                if (mt === 'status') {
                  const state = (m.content as any).execution_state;
                  if (state === 'idle') {
