...
│   └── README.md                    #    快速入门
│
├── jupyterlab-auto-reload/          # JupyterLab 扩展：Notebook 自动刷新（源码）
│   ├── src/index.ts                 #    TypeScript 源码
│   ├── lib/index.js                 #    编译输出
│   ├── jupyterlab_auto_reload/      #    构建产物（含 static/）
│   ├── package.json                 #    npm 依赖与构建脚本
│   ├── tsconfig.json                #    TypeScript 配置
│   └── setup.py                     #    Python 包配置
│
├── jupyterlab-console-adopt/       # JupyterLab 扩展：Console 外部 Session 回显（源码）
│   ├── src/index.ts                 #    TypeScript 源码
│   ├── lib/index.js                 #    编译输出
│   ├── jupyterlab_console_adopt/    #    构建产物（含 static/）
│   ├── package.json                 #    npm 依赖与构建脚本
│   └── tsconfig.json                #    TypeScript 配置
│
...
本仓库包含两个 JupyterLab 4.x 扩展的完整源码，位于项目根目录下：

- `jupyterlab-auto-reload/` — Notebook 自动刷新（MCP 写入后 3s 刷新）
- `jupyterlab-console-adopt/` — Console 外部 Session 回显（.py 源码模式）

每个扩展包含 TypeScript 源码（`src/`）、编译输出（`lib/`）和构建产物（`jupyterlab_*/labextension/`）。
...
jlpm run build     # 编译 TypeScript + 构建 labextension
jupyter labextension install .   # 安装到当前 Jupyter 环境
cd ..
